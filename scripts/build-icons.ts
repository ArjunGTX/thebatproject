...
const typeDir = path.join(cwd, "icons", "types");
const outputDir = path.join(cwd, "public", "icons", "sprites");
await fsExtra.ensureDir(outputDir);
await fsExtra.ensureDir(typeDir);

const generateIconFiles = async () => {
  const spriteFilepath = path.join(outputDir, "default.svg");
  const typeOutputFilepath = path.join(typeDir, "icon-name.ts");
